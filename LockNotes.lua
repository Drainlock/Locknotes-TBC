...
if (arg2 == "SPELL_INTERRUPT") then
	...
	if bit.band(arg5, COMBATLOG_OBJECT_AFFILIATION_MINE) > 0 then
		...
		SendChatMessage("Locked *"..SpellSchool.."* spells for 6sec.", "RAID")
...
-------------------------------CoT and Fear Dispel/wear off----------------------------
if (arg2 == "SPELL_AURA_REMOVED") then
	if bit.band(arg8, COMBATLOG_OBJECT_REACTION_HOSTILE) > 0 then
		if (arg10 == "Curse of Tongues") or (arg10 == "Fear") then
			SpellName = arg10
			Person = arg7
			pat = "-"
   t = {}  -- NOTE: use {n = 0} in Lua-5.0
   fpat = "(.-)" .. pat
   last_end = 1
   local s, e, cap = Person:find(fpat, 1)
   while s do
      if s ~= 1 or cap ~= "" then
	 table.insert(t,cap)
      end
      last_end = e+1
      s, e, cap = Person:find(fpat, last_end)
   end
   if last_end <= #Person then
      cap = Person:sub(last_end)
      table.insert(t, cap)
   end
			ZoneTextString:SetText(""..SpellName.." faded from "..t[1]..".");
			ZoneTextFrame.startTime = GetTime()
			ZoneTextFrame.fadeInTime = 0
			ZoneTextFrame.holdTime = 1
			ZoneTextFrame.fadeOutTime = 2
			...
			PVPInfoTextString:SetText("");
			ZoneTextFrame:Show()
				if (arg10 == "Curse of Tongues") then -- Only plays sounds for CoT, not Fear
					PlaySoundFile("Interface\\AddOns\\LockNotes\\Sounds\\bell.wav");
				end
		end
	end
